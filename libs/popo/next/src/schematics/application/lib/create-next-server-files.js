...
function createNextServerFiles(options) {
    return (host) => {
        if (options.server) {
            const directory = core_1.dirname(core_1.join(options.appProjectRoot, options.server));
            host.create(core_1.join(options.appProjectRoot, options.server), `
      // @ts-check
      'use strict';
      ...
      `);
            const apps = ast_utils_1.appsDir(host);
            host.create(core_1.join(directory, 'server.js'), `
      // @ts-check
      'use strict';
