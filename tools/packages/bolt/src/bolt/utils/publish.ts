...
async function getUnpublishedPackages(packages: any) {
  ...
  for (let pkgInfo of results) {
    let { name, isPublished, localVersion, publishedVersion } = pkgInfo as any;
    if (!isPublished) {
...
export async function publish(
  ...
  let publicPackages = packages.filter((pkg: any) => !pkg.config.getPrivate());
  ...
  try {
    ...
    let unpublishedPackages = publicPackages.filter((pkg: any) => {
      return unpublishedPackagesInfo.some((p: any) => pkg.getName() === p.name);
    });
    ...
    await project.runPackageTasks(unpublishedPackages, spawnOpts, async (pkg: any) => {
